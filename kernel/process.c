  ...
  for( int i=0; i<parent->total_mapped_region; i++ ){
    ...
    switch( parent->mapped_info[i].seg_type ){
      ...
      case CODE_SEGMENT:
        ...
        pte_t *pte = page_walk(parent->pagetable, (uint64)parent->mapped_info[CODE_SEGMENT].va, 0); 
        uint64 parent_pa = PTE2PA(*pte);
        user_vm_map((pagetable_t)child->pagetable, parent->mapped_info[CODE_SEGMENT].va, 
                      parent->mapped_info[CODE_SEGMENT].npages * PGSIZE, parent_pa,
